    ...
    def route_with_decision(
        ...
        if is_multimodal:
            if has_video:
                ...
                # Multimodal image input -> qwen3-vl-4b
                chosen_config = self.registry.get("qwen3-vl-4b")
                routing_reason = (
                    f"Multimodal image input with {complexity.capitalize()} complexity -> "
                    "Selected qwen3-vl-4b (Fast 4B multimodal model)"
                )
        else:
            # Text-Only Inputs -> qwen3-4b
            chosen_config = self.registry.get("qwen3-4b")
            routing_reason = (
                f"Text-only input with {complexity.capitalize()} complexity -> "
                "Selected qwen3-4b (Fast 4B model)"
            )
